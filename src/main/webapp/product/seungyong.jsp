...
if(list.size()==0 && totalCount>0){%>
	...
	location.href = "index.jsp?main=product/productform.jsp?go=seungyong.jsp?currentPage=<%=currentPage -1%>"
	</script>
<%}
%>
<%
...
for(ProductDto dto:list){
	...
	if(dto.getWriter().equals("seungyong")){
%>
...
<%
	for(int pp=startPage;pp<=endPage;pp++)
	{
		...
		{%>
			<li>
			<a href="index.jsp?main=product/productform.jsp?go=seungyong.jsp?currentPage=<%=pp%>"><%=pp %></a></li>
		<%}else{ %>
			<li>
			<a href="index.jsp?main=product/productform.jsp?go=seungyong.jsp?currentPage=<%=pp%>"><%=pp %></a></li>
	...
	if(startPage>1){%>
	<li>
		<a href="index.jsp?main=product/productform.jsp?go=seungyong.jsp?currentPage=<%=startPage-1%>">이전</a></li>
	<%}
	//다음
	if(endPage<totalPage){%>
		<li>
			<a href="index.jsp?main=product/productform.jsp?go=seungyong.jsp?currentPage=<%=endPage+1%>">다음</a></li>
	<%}
	%>
	</ul>
</div> 
