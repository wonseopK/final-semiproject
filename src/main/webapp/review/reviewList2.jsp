...
<%
	...
	// search 총 갯수
	totalCount = dao.getSearchTotalCount(searchCar);
	...
	List<ReviewDto> list = dao.getSearchList(searchCar, start, perPage);
	//List<ReviewDto> list = dao.getReviewlist(start, perpage);


	if (list.size() == 0 && totalCount > 0) // 주의하기!! (무한 마이너스반복)
	{
	%>
		<script type="text/javascript">
			location.href = "index.jsp?main=review/reviewMain.jsp?list=reviewList2.jsp?currentPage=<%=currentPage-1%>";
		</script>
	<%}
...
<div class="bg">
	...
	<%}else{
		...
		{
			if(dto.getCar().equals(searchCar)){
		%>
	...
	<div style="width: 900px;text-align: center; margin-bottom: 50px;">
		...
		if(startPage>1)
		{%>
			<li><a href="index.jsp?main=review/reviewMain.jsp?list=reviewList2.jsp?currentPage=<%=startPage-1%>">이전</a></li>
		<%}
		...
		for(int pp=startPage;pp<=endPage;pp++)
		{
			...
			{%>
				<li class="active"><a href="index.jsp?main=review/reviewMain.jsp?list=reviewList2.jsp?currentPage=<%=pp%>"><%=pp%></a></li>
			<%}else{%>
				<li><a href="index.jsp?main=review/reviewMain.jsp?list=reviewList2.jsp?currentPage=<%=pp%>"><%=pp%></a></li>
			<%}
		}
		...
		if(endPage<totalPage)
		{%>
			<li><a href="index.jsp?main=review/reviewMain.jsp?list=reviewList2.jsp?currentPage=<%=endPage+1%>">다음</a></li>
		<%}%>  
		</ul>
	</div>
